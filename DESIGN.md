# praxis — design & consolidation plan
...
- `praxis-core` — shared `Error`, value types, JSON-CLI helpers, permissions.
- `praxis-browser` / `praxis-desktop` / `praxis-fs` / `praxis-shell` —
  each a lib (+ behavior via the umbrella CLI).
- `praxis-cli` — the `praxis` binary, dispatches domain subcommands.
...
- Umbrella = **praxis** (Greek "action/doing"; pairs with apeiron; distinctive).
  Considered: `computer-control` (plain but generic), `genie` (rejected —
  saturated in AI: Google Genie / Netflix Genie / WSL `genie`; and it
  connotes the assistant/brain layer, not the actuator layer).
- **Drop `adb`/`adc`/`ad*`**: `adb` collides with Android Debug Bridge on
  ...
  vs ai-**desktop**-control). Use domain subcommands: `praxis browser|desktop|fs|shell`.
...
`praxis-shell`, but the bridge enforces the red line by simply not
...
1. **core**: extract shared `Error` / value types / JSON-CLI / permission
   probing from adc & adb into `praxis-core`.
2. Re-home **adc → praxis-desktop** and **adb → praxis-browser** as member
   crates depending on core (public interface unchanged; integrators unaffected).
3. Add **praxis-fs** (thin over `std::fs`) and **praxis-shell**
   (thin over `std::process::Command`), each with fake backends for
   side-effect-free tests.
4. Fill in the `praxis` CLI subcommand trees.
