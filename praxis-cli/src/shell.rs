//! `praxis shell run …` — thin CLI over `praxis-shell`.
//!
...
use clap::Subcommand;
use praxis_shell::{Result, RunRequest, ShellBackend};
use serde_json::{json, Value};

#[derive(Subcommand)]
pub enum Cmd {
    /// Run a program: `praxis shell run [flags] -- prog arg…`, or a script
    /// with `--sh 'cmd | cmd'`.
    Run {
...
pub fn run_with(sh: &dyn ShellBackend, cmd: Cmd) -> Result<Value> {
    ...
    for e in envs {
        let (k, v) = e.split_once('=').ok_or_else(|| {
            praxis_shell::Error::invalid(format!("--env {e:?}: expected KEY=VALUE"))
        })?;
    ...
    let r = sh.run(&req)?;
    let mut v =
        serde_json::to_value(&r).map_err(|e| praxis_shell::Error::internal(e.to_string()))?;
    v["ok"] = json!(r.success());
    Ok(v)
}
