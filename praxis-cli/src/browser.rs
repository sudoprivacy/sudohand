//! `praxis browser <tool> [flags]` — thin CLI over `praxis-browser`, one
//! subcommand per tool, names identical to adb / the Python
//! `ai_dev_browser.tools.<name>` modules (kebab-case aliases too).
//! Output is JSON on stdout; failures use the praxis error envelope.
...
use praxis_browser::actions::TypeOptions;
use praxis_browser::browser::{Reuse, StartOptions};
use praxis_browser::chrome::Headless;
use praxis_browser::connection::{connect_browser, get_active_tab, BrowserClient, Tab};
use praxis_browser::dialog::DialogOptions;
use praxis_browser::elements::{ScrollOptions, TypeByTextOptions};
use praxis_browser::image_cap::ImageCap;
use praxis_browser::mouse::ClickOptions;
use praxis_browser::page::{PdfOptions, ScreenshotOptions};
use praxis_browser::snapshot::DiscoverOptions;
...
pub enum Cmd {
    ...
    PageScreenshot {
        ...
        #[arg(long, default_value_t = praxis_browser::geometry::MAX_SCREENSHOT_LONG_EDGE)]
        max_long_edge: u32,
        /// Total-pixel cap (0 disables)
        #[arg(long, default_value_t = praxis_browser::geometry::MAX_SCREENSHOT_TOTAL_PIXELS)]
...
async fn browser_and_tab(conn: &Conn) -> praxis_browser::Result<(BrowserClient, Tab)> {
    let mut browser = connect_browser(None, conn.port).await?;
    let tab = get_active_tab(&mut browser, conn.tab_url.as_deref()).await?;
    Ok((browser, tab))
}

fn parse_overrides(raw: Option<&str>) -> praxis_browser::Result<Vec<(String, Option<String>)>> {
    let Some(raw) = raw else {
        return Ok(Vec::new());
    };
    let v: Value = serde_json::from_str(raw)?;
    let obj = v.as_object().ok_or_else(|| {
        praxis_browser::Error::Invalid("--override-default-args must be a JSON object".to_string())
    })?;
    Ok(obj
        .iter()
        .map(|(k, v)| {
...
async fn run_async(tool: Cmd) -> praxis_browser::Result<Value> {
    match tool {
        Cmd::BrowserStart {
            port,
            headless,
            ...
            praxis_browser::tools::browser_start(&opts).await
        }
        Cmd::BrowserStop { port, stop_all } => {
            praxis_browser::tools::browser_stop(port, stop_all).await
        }
        Cmd::BrowserList { all_workspaces } => {
            praxis_browser::tools::browser_list(all_workspaces).await
        }
        Cmd::PageGoto {
            conn,
            url,
            ...
            praxis_browser::tools::page_goto(&tab, &url, wait).await
        }
        Cmd::PageDiscover {
            conn,
            text,
            ...
            Ok(serde_json::to_value(
                praxis_browser::tools::page_discover(&tab, &opts).await?,
            )?)
        }
        Cmd::PageScreenshot {
            conn,
            ...
            praxis_browser::tools::page_screenshot(&tab, &opts).await
        }
        Cmd::PageInfo { conn } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::page_info(&tab).await
        }
        Cmd::PageReload { conn, ignore_cache } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::page_reload(&tab, ignore_cache).await
        }
        Cmd::PageWaitUrl {
            conn,
            pattern,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::page_wait_url(
                &tab,
                pattern.as_deref(),
                exact.as_deref(),
                timeout,
        ...
        Cmd::PageWaitElement {
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::page_wait_element(
                &tab,
                text.as_deref(),
                selector.as_deref(),
                timeout,
        ...
        Cmd::PageScroll {
            ...
            praxis_browser::tools::page_scroll(&tab, &opts).await
        }
        Cmd::PagePdf {
            conn,
            path,
            ...
            praxis_browser::tools::page_pdf(&tab, &opts).await
        }
        Cmd::PageEmulateFocus { conn, enabled } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::page_emulate_focus(&tab, enabled).await
        }
        Cmd::FocusByRef { conn, r#ref } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::focus_by_ref(&tab, &r#ref).await
        }
        Cmd::HoverByRef { conn, r#ref } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::hover_by_ref(&tab, &r#ref).await
        }
        Cmd::HighlightByRef {
            conn,
            r#ref,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::highlight_by_ref(&tab, &r#ref, duration).await
        }
        Cmd::HtmlByRef { conn, r#ref } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::html_by_ref(&tab, &r#ref).await
        }
        Cmd::ScreenshotByRef {
            conn,
            r#ref,
            ...
            praxis_browser::tools::screenshot_by_ref(&tab, &r#ref, path.as_deref(), cap.as_ref())
                .await
        }
        Cmd::SelectByRef { conn, r#ref } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::select_by_ref(&tab, &r#ref).await
        }
        Cmd::UploadByRef { conn, r#ref, paths } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::upload_by_ref(&tab, &r#ref, &paths).await
        }
        Cmd::PressKey {
            conn,
            key,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::press_key(&tab, &key, r#ref.as_deref(), modifiers).await
        }
        Cmd::FindByText {
            conn,
            text,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::find_by_text(&tab, &text, interactable_only).await
        }
        Cmd::FindByHtmlId { conn, html_id } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::find_by_html_id(&tab, &html_id).await
        }
        Cmd::FindByXpath { conn, xpath } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::find_by_xpath(&tab, &xpath).await
        }
        Cmd::ClickByHtmlId { conn, html_id } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::click_by_html_id(&tab, &html_id).await
        }
        Cmd::ClickByXpath { conn, xpath } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::click_by_xpath(&tab, &xpath).await
        }
        Cmd::ClickRowByText {
            conn,
            text,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::click_row_by_text(&tab, &text, double, nth, checkbox).await
        }
        Cmd::TypeByText {
            conn,
            name,
            ...
            praxis_browser::tools::type_by_text(&tab, &name, &text, opts).await
        }
        Cmd::SelectText {
            conn,
            text,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::select_text(&tab, &text, to_text.as_deref()).await
        }
        Cmd::TabNew { conn, url } => {
            let mut browser = connect_browser(None, conn.port).await?;
            praxis_browser::tools::tab_new(&mut browser, url.as_deref()).await
        }
        Cmd::TabClose { conn, tab_id } => {
            let mut browser = connect_browser(None, conn.port).await?;
            praxis_browser::tools::tab_close(&mut browser, tab_id).await
        }
        Cmd::CdpSend {
            conn,
            method,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::cdp_send(&tab, &method, params.as_deref()).await
        }
        Cmd::DialogRespond {
            conn,
            action,
            ...
            praxis_browser::tools::dialog_respond(&tab, &opts).await
        }
        Cmd::WindowSet {
            conn,
            width,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::window_set(&tab, width, height, state.as_deref(), focus).await
        }
        Cmd::StorageGet { conn, key } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::storage_get(&tab, key.as_deref()).await
        }
        Cmd::StorageSet {
            conn,
            items,
            ...
            let items: Option<serde_json::Map<String, Value>> = match items {
                Some(raw) => match serde_json::from_str::<Value>(&raw)? {
                    ...
                    _ => {
                        return Err(praxis_browser::Error::Invalid(
            ...
            praxis_browser::tools::storage_set(
                &tab,
                items.as_ref(),
                key.as_deref(),
                value.as_deref(),
        ...
        Cmd::CookiesList { conn, domain } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::cookies_list(&tab, domain.as_deref()).await
        }
        Cmd::CookiesSave {
            conn,
            path,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::cookies_save(&tab, path.as_deref(), pattern.as_deref()).await
        }
        Cmd::CookiesLoad { conn, path } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::cookies_load(&tab, path.as_deref()).await
        }
        Cmd::CookiesImport {
            conn,
            domain,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::cookies_import(&tab, &domain, &browser, user_data_dir.as_deref())
                .await
        }
        Cmd::LoginInteractive { url, cookies_path } => {
            praxis_browser::tools::login_interactive(&url, cookies_path.as_deref()).await
        }
        Cmd::CookiesExtract {
            domain,
            browser,
            ...
            let cookies = praxis_browser::tools::cookies_extract(
                &domain,
                &browser,
        ...
        Cmd::Download { conn, url, path } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::download(&tab, &url, path.as_deref()).await
        }
        Cmd::DownloadLink {
            conn,
            xpath,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::download_link(&tab, &xpath, download_dir.as_deref(), timeout)
                .await
        }
        Cmd::PageWaitReady {
            conn,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            let ready = praxis_browser::tools::page_wait_ready(&tab, timeout, idle_time).await;
            if ready {
                Ok(json!({"ready": true}))
            } else {
        ...
        Cmd::PageHtml { conn, outer } => {
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::page_html(&tab, outer).await
        }
        Cmd::ClickByRef {
            conn,
            r#ref,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::click_by_ref(&tab, &r#ref, human_like).await
        }
        Cmd::ClickByText {
            conn,
            text,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::click_by_text(&tab, &text, timeout, human_like).await
        }
        Cmd::DragByRef {
            conn,
            r#ref,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::drag_by_ref(&tab, &r#ref, to_x, to_y, steps, human_like).await
        }
        Cmd::MouseMove {
            conn,
            x,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            let moved = praxis_browser::tools::mouse_move(
                &tab,
                x,
                y,
                screenshot.as_deref(),
        ...
        Cmd::MouseClick {
            ...
            let clicked =
                praxis_browser::tools::mouse_click(&tab, x, y, screenshot.as_deref(), &opts)
                    .await?;
        ...
        Cmd::MouseDrag {
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            let dragged = praxis_browser::tools::mouse_drag(
                &tab,
                (from_x, from_y),
                (to_x, to_y),
                screenshot.as_deref(),
        ...
        Cmd::TypeByRef {
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::tools::type_by_ref(
                &tab,
                &r#ref,
                &text,
                TypeOptions {
        ...
        Cmd::TabList { conn } => {
            let mut browser = connect_browser(None, conn.port).await?;
            praxis_browser::tools::tab_list(&mut browser).await
        }
        Cmd::TabSwitch { conn, tab_id } => {
            let mut browser = connect_browser(None, conn.port).await?;
            praxis_browser::tools::tab_switch(&mut browser, tab_id).await
        }
        Cmd::JsEvaluate {
            conn,
            expression,
            ...
            let (_b, tab) = browser_and_tab(&conn).await?;
            praxis_browser::page::js_evaluate_in(&tab, &expression, frame.as_deref()).await
        }
        Cmd::Workflows | Cmd::Flow { .. } => unreachable!("handled in run()"),
    }
}

fn parse_vars(vars: Vec<String>) -> praxis_core::Result<std::collections::HashMap<String, String>> {
    let mut map = std::collections::HashMap::new();
    for v in vars {
        let (k, val) = v.split_once('=').ok_or_else(|| {
            praxis_core::Error::invalid(format!("--var {v:?}: expected name=value"))
        })?;
...
/// The workflow commands speak `praxis_core::Error` natively (so a failed
/// flow keeps its real category) — dispatched here, not in `run_async`.
async fn run_flow(cmd: Cmd) -> praxis_core::Result<Value> {
    match cmd {
        Cmd::Workflows => Ok(json!({
            "workflows": praxis_browser::registry::Registry::builtins().list()
        })),
        Cmd::Flow { conn, name, vars } => {
            let map = parse_vars(vars)?;
            let (browser, tab) = browser_and_tab(&conn).await?;
            let runner = praxis_browser::workflow::Runner::new(browser, tab);
            let report = praxis_browser::registry::Registry::builtins()
                .run(&name, &runner, map)
                .await?;
            Ok(json!({"ok": true, "workflow": name, "steps": report}))
        }
        other => run_async(other).await.map_err(praxis_core::Error::from),
    }
}

/// Entry point for `praxis browser <tool>`: a current-thread runtime, the
/// tool's own JSON on success, and the praxis error envelope on failure.
pub fn run(cmd: Cmd) -> praxis_core::Result<Value> {
    let rt = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
        .map_err(|e| praxis_core::Error::internal(format!("tokio: {e}")))?;
