//! `praxis shell run` must not leave its child behind when praxis itself is
...
fn check(sig: &str) {
    let marker = format!("praxis-signal-test-{}-{sig}", std::process::id());
    let mut praxis = Command::new(env!("CARGO_BIN_EXE_praxis"))
        .args([
            "shell",
            "run",
            "--sh",
    ...
    assert!(Command::new("kill")
        .args([&format!("-{sig}"), &praxis.id().to_string()])
        .status()
        .unwrap()
        .success());
    let status = praxis.wait().unwrap();
    assert!(!status.success());
    std::thread::sleep(Duration::from_millis(300));
    let alive = child_alive(&marker);
    let _ = Command::new("pkill").args(["-f", &marker]).status();
    assert!(!alive, "child survived praxis being killed with SIG{sig}");
}
