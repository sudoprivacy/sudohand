...
use praxis_browser::browser::{browser_start, StartOptions};
use praxis_browser::chrome::Headless;
use praxis_browser::connection::{get_active_tab, BrowserClient, Tab};
...
impl Drop for Chrome {
    fn drop(&mut self) {
        let _ = praxis_browser::port::kill_process_tree(self.pid);
        let _ = praxis_browser::port::cleanup_temp_profile(self.port);
    }
}
...
pub async fn start_chrome_in_band() -> Chrome {
    let port = praxis_browser::port::get_available_port((9440, 9450), &[]).expect("in-band port");
...
pub async fn open(chrome: &Chrome, url: &str) -> (BrowserClient, Tab) {
    ...
    praxis_browser::tools::page_goto(&tab, url, true)
        .await
        .expect("goto");
    (browser, tab)
}
