...
use praxis_browser::page::ScreenshotOptions;
use praxis_browser::refs::{node_id_of, parse_ref};
use praxis_browser::snapshot::{DiscoverOptions, Element};
use praxis_browser::tools::*;
...
async fn eval_str(tab: &praxis_browser::connection::Tab, expr: &str) -> String {
    tab.evaluate(expr)
        .await
        .unwrap()
        .as_str()
...
async fn browser_lifecycle_start_list_stop() {
    ...
    assert!(praxis_browser::port::is_port_in_use(chrome.port));
    ...
    let mut free = false;
    for _ in 0..25 {
        if !praxis_browser::port::is_port_in_use(chrome.port) {
            free = true;
            break;
        }
...
async fn screenshot_embeds_scaling_metadata() {
    ...
    let meta = praxis_browser::page::read_screenshot_metadata(&path)
        .unwrap()
    ...
    let (bx, by) = praxis_browser::geometry::scale_coords(ix, iy, Some(&meta));
...
async fn env_port_is_honoured_by_resolve_and_browser_start() {
    ...
    std::env::set_var(praxis_browser::config::PORT_ENV, "61234");
    let start = std::time::Instant::now();
    let p = praxis_browser::connection::resolve_port(None).await;
    assert_eq!(p, 61234);
    assert!(
    ...
    assert_eq!(praxis_browser::connection::resolve_port(Some(7)).await, 7);
    ...
    let want = std::net::TcpListener::bind("127.0.0.1:0")
        ...
        .unwrap()
        .port();
    std::env::set_var(praxis_browser::config::PORT_ENV, want.to_string());
    let r = praxis_browser::tools::browser_start(&praxis_browser::browser::StartOptions {
        headless: Some(praxis_browser::chrome::Headless::New),
    ...
    assert_eq!(praxis_browser::connection::resolve_port(None).await, want);

    // Env port taken → falls back and warns instead of failing silently.
    let r2 = praxis_browser::tools::browser_start(&praxis_browser::browser::StartOptions {
        headless: Some(praxis_browser::chrome::Headless::New),
        startup_timeout: Some(60.0),
        extra_args: std::env::var("ADB_TEST_CHROME_ARGS")
            .map(|v| v.split_whitespace().map(str::to_string).collect())
    ...
    .await
    .unwrap();
    std::env::remove_var(praxis_browser::config::PORT_ENV);
...
async fn storage_dialog_window_cdp_pdf() {
    ...
    // cdp_send raw
    let r = praxis_browser::tools::cdp_send(&tab, "Browser.getVersion", None)
        .await
        .unwrap();
    assert!(r["result"]["product"].as_str().is_some());
    let r = praxis_browser::tools::cdp_send(&tab, "BadMethod", None).await;
    assert!(r.is_err());
    ...
    let r = page_pdf(
        &tab,
        &praxis_browser::page::PdfOptions {
            path: Some(pdf.clone()),
            ..Default::default()
        },
    )
...
async fn cookies_and_screenshot_by_ref_and_js_frame() {
    ...
    .await
    .unwrap();
    let r = praxis_browser::tools::cookies_list(&tab, Some("127.0.0.1"))
        .await
        .unwrap();
    assert!(r["count"].as_u64().unwrap() >= 1);
    let dir = tempfile::tempdir().unwrap();
    let jar = dir.path().join("c.dat");
    let r = praxis_browser::tools::cookies_save(&tab, Some(&jar), None)
        .await
        .unwrap();
    assert_eq!(r["saved"], true);
    assert!(jar.exists());
    let r = praxis_browser::tools::cookies_load(&tab, Some(&jar))
        .await
        .unwrap();
    ...
    let r = praxis_browser::page::js_evaluate_in(&tab, "1+2", None)
        .await
        .unwrap();
