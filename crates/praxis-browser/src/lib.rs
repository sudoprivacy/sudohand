//! `praxis-browser` — the browser actuator, ported from
...
//! library API ([`tools`]); the CLI lives in `praxis-cli`
//! (`praxis browser <tool> [flags]`). Tool names, flags, JSON shapes and
...
//! evaluation details); it converts into `praxis_core::Error` for the
...
/// The tool surface, re-exported by name so `praxis_browser::tools::page_discover`
