#[cfg(unix)]
use sudohand_shell::RealShell;
use sudohand_shell::{FakeShell, RunRequest, RunResult, ShellBackend};
...
mod real {
    ...
    fn via_shell_env_stdin_and_cwd() {
        let sh = RealShell::new();
        let mut r = req("echo \"$SUDOHAND_X:$(pwd):$(cat)\"", &[]);
        r.via_shell = true;
        r.env = vec![("SUDOHAND_X".into(), "42".into())];
        r.cwd = Some("/".into());
