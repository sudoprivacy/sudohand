...
use praxis_fs::{Entry, FakeFs, FsBackend, RealFs};
...
fn apply(fs: &dyn FsBackend, root: &Path, op: &str, a: &Path, b: &Path, data: &[u8]) -> Outcome {
    let r: Result<String, praxis_fs::Error> = match op {
...
fn fake_and_real_agree_on_random_sequences() {
    ...
    for seed in 1..=32u64 {
        let real_root: PathBuf =
            std::env::temp_dir().join(format!("praxis-fs-diff-{}-{seed}", std::process::id()));
