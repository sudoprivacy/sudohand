...
use sudohand_fs::{EntryKind, FakeFs, FsBackend, RealFs};
...
fn real_read_refuses_a_fifo_instead_of_blocking() {
    let dir = std::env::temp_dir().join(format!("sudohand-fs-fifo-{}", std::process::id()));
...
fn real_backend() {
    let root: PathBuf =
        std::env::temp_dir().join(format!("sudohand-fs-test-{}", std::process::id()));
